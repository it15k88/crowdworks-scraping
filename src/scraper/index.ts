...
import { processScrapedJob } from "@/controllers";
          ...
          try {
            ...
            jobs = await page!.evaluate(() => {
              ...
              cardNodes.forEach((card) => {
                ...
                // Price (reward)
                const price = card
                  .querySelector("div.AIu_G")?.textContent?.trim()
                  ? `${card.querySelector("span.dZ4gx")?.textContent?.trim()}  ${card.querySelector("b.L25cC")?.textContent?.trim()}`
                  : card.querySelector("div.zTNhw")?.textContent?.trim()
                    ? `${card.querySelector("span.A04qg")?.textContent?.trim()}  ${card.querySelector("b.rp0CH")?.textContent?.trim()}`
                    : "";
